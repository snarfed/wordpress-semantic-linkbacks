...
class Linkbacks_Handler {
	...
	public static function init() {
		...
		add_filter( 'comment_moderation_text', array( 'Linkbacks_Handler', 'comment_moderation_text' ), 10, 2 );
		add_filter( 'comment_moderation_subject', array( 'Linkbacks_Handler', 'comment_moderation_subject' ), 10, 2 );
		add_filter( 'comment_notification_text', array( 'Linkbacks_Handler', 'comment_notification_text' ), 10, 2 );
		add_filter( 'comment_notification_subject', array( 'Linkbacks_Handler', 'comment_notification_subject' ), 10, 2 );
		add_filter( 'ckpn_newcomment_subject', array( 'Linkbacks_Handler', 'comment_notification_subject' ), 10, 2 );
		add_filter( 'ckpn_newcomment_message', array( 'LinkbackS_Handler', 'basic_notification_text' ), 10, 2 );
		add_filter( 'fnpn_newcomment_subject', array( 'Linkbacks_Handler', 'comment_notification_subject' ), 10, 2 );
		add_filter( 'fnpn_newcomment_message', array( 'LinkbackS_Handler', 'basic_notification_text' ), 10, 2 );

		if ( WP_DEBUG ) {
			// For testing outgoing comment email
			add_filter( 'bulk_actions-edit-comments', array( 'Linkbacks_Handler', 'register_bulk_send' ) );
			add_filter( 'handle_bulk_actions-edit-comments', array( 'Linkbacks_Handler', 'bulk_send_handler' ), 10, 3 );
		}
	 ...
	 *
	 */
	public static function bulk_send_handler( $redirect_to, $doaction, $comment_ids ) {
	...
	*/
	...
	public static function basic_notification_text( $notify_message, $comment_id ) {
		...
		$type         = self::get_comment_type_strings( $type );
	...
	public static function notification_body( $comment ) {
		...
		$notify_text[] = sprintf( __( 'Author: %1$1s(%2$2s) ', 'semantic-linkbacks' ), get_comment_author( $comment ), self::get_author_url( $comment ) );
		/* translators: Semantic Linkback comment type string */
		$notify_text[] = sprintf( __( 'Semantic Type: %s', 'semantic-linkbacks' ), $type );
		/* translators: URL */
		$notify_text[] = sprintf( __( 'URL: %s ', 'semantic-linkbacks' ), self::get_url( $comment ) );
		...
		$notify_text[] = sprintf( __( 'You can see all responses to this post here: %1s#comments', 'semantic-linkbacks' ), get_permalink( $comment->comment_post_ID ) );
